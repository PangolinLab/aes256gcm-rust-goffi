...
const KEY_HEX_LENGTH: usize = 64;       // 32 bytes -> 64 hex chars
const NONCE_HEX_LENGTH: usize = 24;     // 12 bytes -> 24 hex chars
const KEY_BYTES_LENGTH: usize = 32;
const NONCE_BYTES_LENGTH: usize = 12;
...
/// The returned pointer must be freed using [aes_256_gcm_free].
///
/// # Safety
/// - Inputs must be null-terminated C strings.
/// - Caller must ensure inputs are valid UTF-8 and hex-encoded.
/// - Returned pointer must be freed with [aes_256_gcm_free] to avoid memory leaks.
#[unsafe(no_mangle)]
pub extern "C" fn aes_256_gcm_encrypt(
    ...
    if key_str.len() != KEY_HEX_LENGTH ||
       nonce_str.len() != NONCE_HEX_LENGTH ||
       pt_str.len() % 2 != 0 {
    ...
    if key.len() != KEY_BYTES_LENGTH {
        return std::ptr::null_mut(); // Invalid key length
    }
    if nonce.len() != NONCE_BYTES_LENGTH {
